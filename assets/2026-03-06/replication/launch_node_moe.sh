#!/bin/bash
# Launch a single node for the MoE benchmark
# Usage: bash launch_node_moe.sh <num_nodes> <node_rank> <master_addr> <master_port> <ib_disable> <model_path> [seq_len] [batch_size] [steps]
#
# Key fix: NCCL_IB_DISABLE alone does not work with NCCL 2.28+ external RDMA plugin.
...
MASTER_PORT=$4
IB_DISABLE=${5:-0}
MODEL_PATH=${6:-/lustre/models/Mixtral-8x7B-v0.1}
SEQ_LEN=${7:-2048}
BATCH_SIZE=${8:-1}
STEPS=${9:-20}
GPUS_PER_NODE=8
...
echo "Launching MoE node rank $NODE_RANK / $NUM_NODES"
...
sudo docker run --rm \
    ...
    bash -c "pip install transformers==4.47.1 accelerate -q && \
    torchrun \
        ...
        /lustre/scripts/finetune_bench_moe.py \
